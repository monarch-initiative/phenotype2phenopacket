...
from phenotype2phenopacket.convert import write_phenopacket
...
def filter_for_disease(variant_summary_df: pl.DataFrame, disease: Disease) -> pl.DataFrame:
    print(disease.term.id)
    return variant_summary_df.filter(
        pl.col("PhenotypeIDS").str.contains(r"\b" + re.escape(disease.term.id) + r"\b")
    )
...
def filter_for_clinical_significance(
    variant_summary_df: pl.DataFrame, clinical_significance_filter: int
):
    mapped_clinical_significance = variant_summary_df.select(
        [
            pl.col("ClinicalSignificance").apply(lambda x: map_clinical_significance(x)),
    ...
    return mapped_clinical_significance.filter(
        pl.col("ClinicalSignificance") >= clinical_significance_filter
    )


def filter_for_genome_assembly(variant_summary_df, genome_assembly_filter):
    return variant_summary_df.filter(pl.col("Assembly") == genome_assembly_filter)


def filter_variant_summary(
    variant_summary_df, disease, clinical_significance_filter, genome_assembly_filter
):
    disease_variant_summary = filter_for_disease(
        variant_summary_df=variant_summary_df, disease=disease
    )
    if len(disease_variant_summary) == 0:
        print("No disease matches")
        return None
    filtered_variants = filter_for_clinical_significance(
        disease_variant_summary, clinical_significance_filter
    )
    return filter_for_genome_assembly(filtered_variants, genome_assembly_filter)
...
def add_variant_interpretations(
    ...
    filtered_variant_summary = filter_variant_summary(
        variant_summary_df, disease, clinical_significance_filter, genome_assembly_filter
    )
