...
from phenopackets import (
    Resource, MetaData, Individual, PhenotypicFeature, OntologyClass, Disease, Phenopacket,
    TimeElement
)
...
def create_onset(human_phenotype_ontology, phenotype_annotation_entry: dict) -> TimeElement:
    if phenotype_annotation_entry["onset"] is not None:
        ...
        return TimeElement(ontology_class=OntologyClass(id=phenotype_annotation_entry["onset"], label=term))
    else:
        return None
...
def create_phenotypic_feature(human_phenotype_ontology, phenotype_annotation_entry: dict) -> PhenotypicFeature:
    if phenotype_annotation_entry["aspect"] == "P":
        ...
        return PhenotypicFeature(type=OntologyClass(id=phenotype_annotation_entry["hpo_id"], label=hpo_term),
...
def create_disease(phenotype_annotation_entry: dict) -> Disease:
    return Disease(term=OntologyClass(id=phenotype_annotation_entry["database_id"],
...
def create_metadata() -> MetaData:
    ...
    return MetaData(created=timestamp,
...
def convert_to_phenopackets(phenotype_annotation: pl.DataFrame, output_dir: Path):
    ...
    for omim_disease in grouped_omim_diseases:
        ...
        for phenotype in omim_disease.rows(named=True):
            phenotypic_feature = create_phenotypic_feature(human_phenotype_ontology=human_phenotype_ontology,
        ...
        write_phenopacket(phenopacket, output_dir.joinpath(
